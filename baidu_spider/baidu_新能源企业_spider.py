...
def dealHtml(html):
    ...
    with open(os.path.join("../baidu_新能源企业_news", fileName), 'a+', encoding='utf-8-sig', newline='') as f:
        writer = csv.writer(f)
        for row in saveData:
...
def doSpider(keyword, sortBy='focus'):
    ...
    if not os.path.exists(os.path.join("../baidu_新能源企业_news")):
        os.mkdir(os.path.join("../baidu_新能源企业_news"))
    if not os.path.exists(fileName):
        with open(os.path.join("../baidu_新能源企业_news", fileName), 'w+', encoding='utf-8-sig', newline='') as f:
            writer = csv.writer(f)
...
if __name__ == "__main__":
    ...
    for law_entity_one in law_entity:
        law_entity_one = law_entity_one[0].strip("\n")
        doSpider(keyword=law_entity_one, sortBy='focus')
